...
class VariableOrderMarkovMIDIGenerator:
        ...
        for msg in mido.merge_tracks(mid.tracks):
            if msg.type == "note_on" and msg.velocity > 0:
                if len(previous_notes) >= self.order:
                    key = tuple(previous_notes[-self.order :])
    ...
    def generate_sequence(self, length):
        ...
        for _ in range(length - len(start_notes)):
            current_key = tuple(sequence[-self.order :])
    ...
    def generate_midi(self, sequence, output_file):
        ...
        for i in range(len(sequence)):
            key = tuple(sequence[max(0, i - self.order) : i])
            note = sequence[i]
    ...
    def load_model(self, file_path):
        ...
        try:
            with open(file_path, "r") as f:
                data = json.load(f)
                self.order = data.get("order", 2)  # Default to 2 if not specified
                self.model = {
    ...
    def visualize_markov_chain(self, top_n=10):
        ...
        plt.title(
            f"Top {top_n} Transitions in Markov Model of MIDI Note Transitions (Order: {self.order})"
        )
        plt.show()
    ...
    def create_transition_heatmap(self, top_n=100):
        ...
        plt.title(
            f"Transition Probabilities for Top {top_n} States (Order: {self.order})"
        )
    ...
    def summarize_model(self, top_n=5):
        ...
        print(
            f"Model Summary (Order: {self.order}, Top {top_n} transitions for each state):"
        )
        print("---------------------------------------------------")
...
if __name__ == "__main__":
    ...
    for order in range(1, 6):
        generator = VariableOrderMarkovMIDIGenerator(order=order)
        model_path = get_unique_filename(
            f"../output/markov_models/markov_3_order_{order}.json"
        )
        ...
        if not generator.model:
            ...
            try:
                ...
                output_file = get_unique_filename(
                    f"../output/midi/markov_3_order_{order}.mid"
                )
